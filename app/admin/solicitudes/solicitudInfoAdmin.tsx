...
export default function solicitudInfoAdmin() {
    const { solicitudId } = useLocalSearchParams();

    const reclamoId = solicitudId ? parseInt(solicitudId as string) : null;
    ...
    // Convierte la imagen codificada en Base64 para mostrarla.
    const getDecodedImage = () => {
        if (reclamo.fotoReclamo) {
            return `data:image/jpeg;base64,${reclamo.fotoReclamo}`;
        }
        return null;
    };
        ...
        <SafeAreaView style={styles.container}>
                    ...
                    (reclamo.fotoReclamo ? (
                        <View style={styles.card}>
                            ...
                            <TouchableOpacity onPress={() => setImageModalVisible(true)}>
                                <Image source={{ uri: getDecodedImage() }} style={styles.image} resizeMode="contain" />
                            </TouchableOpacity>
                        </View>
                    ) : (
                        <View style={styles.card}>
            ...
            <Modal visible={isImageModalVisible} transparent={true} animationType="fade">
                <View style={styles.modalContainer}>
                    ...
                    <Image source={{ uri: getDecodedImage() }} style={styles.modalImage} resizeMode="contain" />
                </View>
            </Modal>
        </SafeAreaView>
    );
